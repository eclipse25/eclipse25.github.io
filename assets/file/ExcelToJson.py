...
if args.verbose >= 1:
    ...
    if args.drop_null == True:
        print("Drop NULL value in tags: Yes")
    if args.dropmode == 0:
...
excel_raw = pd.read_excel(args.filename, sheet_name=args.sheetname, usecols=['name', 'address', 'category','trav_time','place', 'avg_Price','image','MapLink','NaverMap','OneLiner'])
...
if args.drop_null == True:
    if args.verbose >= 1:
        print("Dropping NULL values in tags...")
    
    for i in range(len(TagList['tags'])):
        TempList = []
        for j in range(len(TagList['tags'][i])):
            if type(TagList['tags'][i][j]) == float and np.isnan(TagList['tags'][i][j]):
                pass
            else:
                TempList.append(TagList['tags'][i][j])
        TagList['tags'][i] = TempList
            
        #TagList['tags'][i].dropna(inplace = True)
...
#'name', 'address', 'category','trav_time','opendays','image','MapLink','OneLiner'
    ColOrder = ["name", "address", "category", "trav_time","tags", "opentime","closetime","breaktime","opendays","image","MapLink", "OneLiner"]
...
# ----- MODE 1 ----- #
    ...
    ColOrder = ["name", "address", "category", "trav_time", "tags", "times","opendays","image","MapLink", "OneLiner"]
    FinalList = FinalList[ColOrder]
...
# ----- MODE 3 ----- #
    ...
    TimeList['times'] = TimeList[['opentime','breaktime_start','breaktime_end','closetime']].values.tolist()
    
    for i in range(len(TimeList['times'])):
        TempList = []
        for j in range(len(TimeList['times'][i])):
            if type(TimeList['times'][i][j]) == float and np.isnan(TimeList['times'][i][j]):
                pass
            else:
                TempList.append(TimeList['times'][i][j])
        TimeList['times'][i] = TempList
    ...
    ColOrder = ["name", "address", "category", "trav_time","place","avg_Price", "tags", "times","image","MapLink","NaverMap", "OneLiner"]
    FinalList = FinalList[ColOrder]
